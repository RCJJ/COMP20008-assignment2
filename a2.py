...
def contacts_num():
    ...
    plt.bar(arange(len(status))-0.2,vic_number, width=0.4)
    plt.bar(arange(len(status))+0.2,all_number, width=0.4,color='r')
    plt.xticks(arange(len(status)),status)
...
def contacts_rate():
    ...
    plt.bar(arange(len(status))-0.2,vic_rate, width=0.4)
    plt.bar(arange(len(status))+0.2,all_rate, width=0.4,color='r')
    plt.xticks(arange(len(status)),status)
    plt.legend(["Victoria", "National total"])
    plt.show()
...
def compare():
    ...
    plt.plot(line1, 'bo-', linewidth=2.0, label='total_indigenous')
    plt.plot(line2, 'go-', linewidth=2.0, label='total_non-indigenous')
    plt.plot(line3, 'ro-', linewidth=2.0, label="VIC_indigenous")
    plt.plot(line4, 'mo-', linewidth=2.0, label='VIC_non-indigenous')
    plt.xlabel('year')
